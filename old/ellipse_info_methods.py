...
def method_2_1(
    ...
    for i, coord in enumerate(particle):
        # Maps greyscale value based on assumption of black particle. It is now a proportion.
        const = 1  # The proportion in question
        greyscale_value = microscope_image_cropped[coord]
        excess_area += min(1, const * (greyscale_value / (255 - av_grey))) * (
            um_per_pixel**2
        )  # uint8
...
def method_2_3(
    ...
    for i, coord in enumerate(particle):
        ...
        const = 300  # The proportion in question
